...
class GlobalAnalyzer:
    ...
    def _load_stock_aliases(self):
        ...
        alias_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "stock_aliases.json")
    ...
    def get_global_win_rate(self, min_mentions: int = 2,
        ...
        for item in raw_data:
            ...
            avg_benchmark_return = sum(item['detail_benchmark_returns'][i] for i, r in enumerate(item['detail_returns']) if r is not None and i < len(item['detail_benchmark_returns']) and item['detail_benchmark_returns'][i] is not None) / len([x for x in item['detail_benchmark_returns'] if x is not None]) if len([x for x in item['detail_benchmark_returns'] if x is not None]) > 0 else 0.0
